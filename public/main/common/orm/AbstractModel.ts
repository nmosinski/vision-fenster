...
abstract class AbstractModel<T extends AbstractModel<T>> implements IComparable
{
     ...
     */
    constructor (data?: object, dontBoot = false)
    {
     ...
     */
    static dummy<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U, customChanges?: KVMap<string, any>): U
    {
        let u = new Model();
        const item = {};
    ...
    {
        let relation: Relation<U, T>;
        try
        {
            relation = this.relations.get(Relative) as Relation<U, T>;
        } catch (err)
        {
    ...
    /**
     * Get a relative as fk for this.
     * @param {new()=>U, extends AbstactModel<U>} Relative The class obejct/constructor of the relative.
     * @returns {string} The relative as fk.
     */
    public relativeAsFk<U extends AbstractModel<U>>(Relative: new () => U): string
    {
        return this.getRelation(Relative).aAsFkForB();
    }

    /**
     * Get a relative as property name for this.
     * @param {new()=>U, extends AbstactModel<U>} Relative The class obejct/constructor of the relative.
     * @returns {string} The relative as property name.
     */
    public relativeAsPropertyName<U extends AbstractModel<U>>(Relative: new () => U): string
    {
        return this.getRelation(Relative).aAsPropertyNameForB;
    }

    /**
     * Add a relation.
     * @param {Relation<U,T>} relation The relation to be added. 
     */
    addRelation<U extends AbstractModel<U>>(relation: Relation<U, T>): void
    {
        this.relations.add(relation.relativeA, relation);
        this.properties.string(relation.aAsFkForB());
    }
     ...
     * @param {string} [fkColumnName] The name of the foreign primary key column name.
     */
    zeroOrOneToOne<U extends AbstractModel<U>>(Model: new () => U, relativeAsPropertyName: string = null): void
    {
        // This logic inverse.
        this.addRelation(new OneToZeroOrOne(Model, this.Constructor, relativeAsPropertyName));
     ...
     */
    oneToZeroOrOne<U extends AbstractModel<U>>(Model: new () => U, relativeAsPropertyName: string = null): void
    {
        // This logic inverse.

        this.addRelation(new ZeroOrOneToOne(Model, this.Constructor, relativeAsPropertyName));
    }
     ...
     */
    oneToMany<U extends AbstractModel<U>>(Model: new () => U, relativeAsPropertyName: string = null): void
    {
        // This logic, inverse.

        this.addRelation(new ManyToOne(Model, this.Constructor, relativeAsPropertyName));
    }
     ...
     * @param {string} [fkColumnName] The name of the foreign primary key column name.
     */
    manyToOne<U extends AbstractModel<U>>(Model: new () => U, relativeAsPropertyName: string = null): void
    {
        // This logic, inverse.
        this.addRelation(new OneToMany(Model, this.Constructor, relativeAsPropertyName));
    }

    /**
     * @todo add possibility to add property names.
     ...
     */
    manyToMany<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U, relativeAsPropertyName: string = null, thisAsPropertyName: string = null): void
    {
        const thisTableName = this.tableName;
        const relativeTableName = AbstractModel.tableName(Model);

        const thisAsFk = (thisAsPropertyName) ? thisAsPropertyName + 'Id' : thisTableName.charAt(0).toLowerCase() + thisTableName.slice(1) + 'Id';
        const relativeAsFk = (relativeAsPropertyName) ? relativeAsPropertyName + 'Id' : relativeTableName.charAt(0).toLowerCase() + relativeTableName.slice(1) + 'Id'
        ...
        const roleModelClass = class RoleModel extends AbstractModel<RoleModel> implements IComparable
        {
            ...
            equals(object: any): boolean
            {
                ...
                const thisIdA = this[relativeAsFk];
                const objectIdA = object[relativeAsFk];
        ...
        this.addRelation(new ManyToMany(Model, this.Constructor, roleModelClass, relativeAsPropertyName));
    }
    ...
    async find(): Promise<QueryResult<T>>
    {
        ...
        if (this.parent)
        {
            ...
            propertyName = relationToParent.bAsPropertyNameForA;
    ...
    static asSinglePropertyName<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U): string
    {
        return (new Model(null, true)).asSinglePropertyName();
    }
    ...
    static asMultiplePropertyName<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U): string
    {
        return (new Model(null, true)).asMultiplePropertyName();
    }
    ...
    static tableName<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U): string
    {
        return (new Model(null, true)).tableName;
    }

    static modelName<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U): string
    {
        return (new Model(null, true)).modelName;
    }

...
class WixDatabase<T extends AbstractModel<T>>
{
     ...
     */
    static async get<U extends AbstractModel<U>>(id: string, Model: new (data?: object, dontBoot?: boolean) => U): Promise<U>
    {
        const item = await wixData.get(AbstractModel.tableName(Model), id);
        if (!item)
     ...
     * @returns {boolean} true if the item exists, else false.
     */
    static async has<U extends AbstractModel<U>>(id: string, Model: new (data?: object, dontBoot?: boolean) => U): Promise<boolean>
    {
        const item = await wixData.get(AbstractModel.tableName(Model), id);
        if (!item)
            return false;
     ...
     */
    static async removeAll<U extends AbstractModel<U>>(Model: new (data?: object, dontBoot?: boolean) => U): Promise<void>
    {
        return await wixData.truncate(AbstractModel.tableName(Model));
    }
...
export class Query<T extends AbstractModel<T>>
{
     ...
     */
    constructor (Model: new (data?: object, dontBoot?: boolean) => T)
    {
        this.Model = Model;
        this.query = wixData.query(AbstractModel.tableName(Model));
    }

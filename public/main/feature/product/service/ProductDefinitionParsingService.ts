...
class ProductDefinitionParsingService {
    static parseFromJson(json: object): KVMap<string, ProductDefinition> {
        return ProductDefinitionParsingService.parse(json);
    }

    private static parse(productDefObj: object): KVMap<string, ProductDefinition> {
        let productDefinitions = new KVMap<string, ProductDefinition>();
        for (var key1 in productDefObj) {
            ...
            productDefinitions.add(productDefinition.model, productDefinition);
        }

...
class ManyToMany<A extends AbstractModel<A>, B extends AbstractModel<B>> extends Relation<A, B>
{
    ...
    constructor (relativeA: new () => A, relativeB: new () => B, roleModel: new () => AbstractModel<any>, aAsPropertyNameForB: string | null = null)
    {
    ...
    {
        return new this.relativeA().getRelation(this.relativeB) as ManyToMany<B, A>;
    }
    ...
    async assign(bs: AnyNumber<B>, as: AnyNumber<A>): Promise<void>
    {
        ...
        bsList.foreach((b: B) =>
        {
            asList.foreach((a: A) =>
            {
                ...
                const aAsFk = this.aAsFkForB();
                const bAsFk = this.bAsFkForA();
    ...
    areRelated(a: A, b: B, roles: AnyNumber<AbstractModel<any>>): boolean
    {
        ...
        {
            if (role[this.aAsFkForB()] === a.id && role[this.bAsFkForA()] === b.id)
                ret = true;
        });
